...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 10 16:13:05 2019
...
"""
190110_Dssat spil-water relation file
http://ec2-52-196-202-21.ap-northeast-1.compute.amazonaws.com/cropsim/exec/2019-01-10T07-08-16-776Z3cf67411f5dd4fba
soilid -> "JP_NRPH10F"
...
soilW = []
with open(os.getcwd() + '/2019-01-10T07-08-16-776Z3cf67411f5dd4fba/SoilWat.OUT', 'r') as f:
    for row in f:
...
sw_df7T = pd.DataFrame(s_stress[:, 1:], columns=columns[1:], index = newcol)
 
"""
190108 Summary.OUT
"""
record = []
with open(os.getcwd() + '/2019-01-10T07-08-16-776Z3cf67411f5dd4fba/Summary.OUT', 'r') as f:
    for row in f:
...
sum_df7T = pd.DataFrame(summary[:, 1:], index=summary[:, 0], columns=col[1:])

"""
190108 SoilNi.OUT read Soil N Dssat file.
"""
soilN = []
with open(os.getcwd() + '/2019-01-10T07-08-16-776Z3cf67411f5dd4fba/SoilNi.OUT', 'r') as f:
    for row in f:
...
sn_df7T = pd.DataFrame(sn_stress[:, 1:], columns=columns[1:], index = newcol) 
...
PN = []
with open(os.getcwd() + '/2019-01-10T07-08-16-776Z3cf67411f5dd4fba/PlantN.OUT', 'r') as f:
    for row in f:
        PN.append(row.strip())
...
pn_df7T = pd.DataFrame(pn_stress[:, 1:], columns=columns[1:], index = newcol)
...
PG = []
with open(os.getcwd() + '/2019-01-10T07-08-16-776Z3cf67411f5dd4fba/PlantGro.OUT', 'r') as f:
    for row in f:
        PG.append(row.strip())
...
pg_df7T = pd.DataFrame(pg_stress[:, 1:].astype(np.float32), columns=columns[1:], index = newcol)
