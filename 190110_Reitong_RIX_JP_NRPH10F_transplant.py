...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 10 07:23:06 2019
...
http://ec2-52-196-202-21.ap-northeast-1.compute.amazonaws.com/cropsim/exec/2019-01-09T22-14-43-673Ze7e3e8533b0b2927
soilid -> "JP_NRPH10F"
planting method -> transplanting
transplanting date -> 18214
...
"""
info = sum_df5T.loc[:, ["HDAT","HWAH", "PRCP"]].values.astype(np.float32)
sum_df5T_1 = pd.DataFrame(info, index=sum_df5T.index, columns=["HDAT","HWAH", "PRCP"])

order1T = sum_df5T_1.sort_values("HWAH")["HWAH"].index

juv_no1 = sn_df5T[sn_df5T["DOY"]=="225"]
uni = []
...
hwah4 = sum_df5T.loc[:, 'HWAH'].values.astype(np.int32)   #sum_df1.sort_values("HWAH")
...
for i in range(100):
    print(juv_no1_1.sort_values("NI1D").index[i],
          sum_df5T.loc[repr(int(juv_no1_1.sort_values("NI1D").index[i])),"HWAH"])
...
dfs = sn_df5T[sn_df5T.index == wnum]
dfp = pn_df5T[pn_df5T.index == wnum]
...
plt.title("N stream between soil and plant in No" + repr(wnum)+"(yield="+sum_df5T.loc[repr(wnum),"HWAH"]+"kg/ha)", fontsize=18)
...
dttd = []
for i in range(100):
    df = pg_df5T[pg_df5T.index == i+1]
    dd = sum(pg_df5T[pg_df5T.index == i+1].query("215<DOY<320")["DTTD"].values)
    dttd.append(dd)
...
hwah4 = sum_df5T.loc[:, 'HWAH'].values.astype(np.int32)
...
plt.legend(loc='best', fontsize=15)
plt.title("Degree-Days during DOY 215-305 VS Yield", fontsize=18)
plt.xlabel('Degree-Days during DOY 215-305 (average-celcius*day)', fontsize=16)
plt.ylabel("Final Yield (kg/ha)", fontsize=16)
plt.setp(ax.get_xticklabels(), fontsize=15, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=15, visible=True)
#plt.savefig('190108_Reiton_RIX_png/190110_Degree-days_DOY215_320_VS_Yield_JP_NRPH10F_Transplant.png', bbox_inches='tight')
...
plt.setp(ax.get_yticklabels(), fontsize=15, visible=True)
#ax.set_xlim(-10, 200)
#plt.savefig('190108_Reiton_RIX_png/181226_Precipitation_DAS60_70_VS_Yield_SSKT8605.png', bbox_inches='tight')

plt.show()


#190110 Degree-days vs precipitation during cultivation
fig = plt.figure(figsize = (11, 8))
ax = fig.add_subplot(1,1,1)

dttd = np.asarray(dttd)
prcpA = sum_df5T.loc[:, 'PRCP'].values.astype(np.float32)
...
ax.scatter(dttd, prcpA, s=40, c=sum_df5T.loc[:, 'HWAH'].values.astype(np.float32), cmap=cm.Reds)
...
#plt.savefig('190108_Reiton_RIX_png/190110_Degree-days_DOY215_320_VS_Precipitation_JP_NRPH10F_Transplant.png', bbox_inches='tight')

plt.show()


#190110 Degree-days vs solar radiation during cultivation

srad = []
for i in range(100):
    df = searchElementforX(wlis[i], "^18")
    sr = sum(df.loc["18215":"18321", "SRAD"])
    srad.append(sr)
...
ax.scatter(dttd, srad, s=40, c=sum_df5T.loc[:, 'HWAH'].values.astype(np.float32), cmap=cm.Reds)

for a, b, c in zip(np.arange(1,101),dttd,srad):
    ax.annotate(a, xy=(b,c),size=11)
    
ax.plot(X2, clf.predict(X2), color = 'blue', label='R2={:.3f}'.format(clf.score(X2,Y2)))

plt.legend(loc='best', fontsize=15)
plt.title("Degree-Days during DOY 215-320 VS Solar radiation during cultivation", fontsize=18)
plt.xlabel('Degree-Days during DOY 215-320 (average-celcius*day)', fontsize=16)
plt.ylabel("Solar radiation amount(MJ/m2)", fontsize=16)
plt.setp(ax.get_xticklabels(), fontsize=15, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=15, visible=True)
...
#190110 Precipitation vs solar radiation during cultivation
fig = plt.figure(figsize = (11, 8))
ax = fig.add_subplot(1,1,1)
...
clf = linear_model.LinearRegression()
X2 = prcpA.reshape(-1, 1)
Y2 = srad.reshape(-1, 1)

clf.fit(X2, Y2)

ax.scatter(prcpA, srad, s=40, c=sum_df5T.loc[:, 'HWAH'].values.astype(np.float32), cmap=cm.Reds)

for a, b, c in zip(np.arange(1,101),prcpA,srad):
    ax.annotate(a, xy=(b,c),size=11)
    
ax.plot(X2, clf.predict(X2), color = 'blue', label='R2={:.3f}'.format(clf.score(X2,Y2)))

plt.legend(loc='best', fontsize=15)
plt.title("Precipitation during during cultivation VS Solar radiation during cultivation", fontsize=18)
...
for i in range(100):
    df = searchElementforX(wlis[i], "^18")
    sr = sum(df.loc["18235":"18265", "SRAD"])
    srad.append(sr)
...
ax.scatter(srad, hwah4, s=20, c='red')

for a, b, c in zip(np.arange(1,101),srad,hwah4):
    ax.annotate(a, xy=(b,c),size=11)
    
ax.plot(X2, clf.predict(X2), color = 'blue', label='R2={:.3f}'.format(clf.score(X2,Y2)))

plt.legend(loc='best', fontsize=15)
plt.title("Solar radiation amount during DOY 215-305 VS Yield", fontsize=18)
plt.xlabel('Solar radiation amount(MJ/m2)', fontsize=16)
plt.ylabel("Final Yield (kg/ha)", fontsize=16)
plt.setp(ax.get_xticklabels(), fontsize=15, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=15, visible=True)
#plt.savefig('190108_Reiton_RIX_png/190110_Degree-days_DOY215_320_VS_Yield_JP_NRPH10F_Transplant.png', bbox_inches='tight')
...
for i in range(7):
    wnum = int(order1T[i])
    swtd = sw_df5T.loc[:, 'SWTD'][sw_df5T.index == wnum].values.astype(np.float32)
    doy = sw_df5T.loc[:, 'DOY'][sw_df5T.index == wnum].values.astype(np.int32)
...
#plt.setp(ax2.get_yticklabels(), fontsize=13, visible=True)
#ax.set_ylim(0, 30)
#plt.savefig('190108_Reiton_RIX_png/190110_N_flow_in_1993_SSKT9605.png', bbox_inches='tight')
plt.show()
...
doy = sw_df5T.loc[:, 'DOY'].values.astype(np.int32).reshape(-1, 1)
sw2d = sw_df5T.loc[:,['SW1D', 'SW2D', 'SWTD']].values.astype(np.float32)
swdoy = np.concatenate((doy,sw2d), axis=1)
swdoy_dfT = pd.DataFrame(swdoy, index=sw_df5T.index, columns=["DOY", 'SW1D', 'SW2D',"SWTD"])
wc = []
for i in range(1, 101):
    df = swdoy_dfT[swdoy_dfT.index == i]
...
sday = []
for i in range(1,101):
    df = swdoy_dfT[swdoy_dfT.index == i].query("280<DOY<300")
    n = 0
...
sday2 = []
for i in range(1,101):
    df = swdoy_dfT[swdoy_dfT.index == i].query("199<DOY<210")
    n = 0
    for j in range(len(df.index)):
        if df.iloc[j, 3] < 220:
            n = n+1
...
dfw = sw_df5T[sn_df5T.index == wnum]
dfs = sn_df5T[sn_df5T.index == wnum]
dfp = pn_df5T[pn_df5T.index == wnum]
prep = searchElementforX(wlis[wnum-1], '^18')["RAIN"].values
ax.plot(dfs.loc[:, 'DOY'].values.astype(np.int32), 
...
"""
190110 3D scatter plot of the relationship among Degree-day, SRAD amount and PRCP
"""
dttd = []
for i in range(100):
    df = pg_df5T[pg_df5T.index == i+1]
...
ax.scatter(dttd, srad, prcpA, s=40, c=hwah4, cmap=cm.Reds)

for a, b, c, d in zip(np.arange(1,101),dttd, srad, prcpA):
    ax.text(b, c, d, a)
#ax.plot(X2, clf.predict(X2), color = 'blue', label='R2={:.3f}'.format(clf.score(X2,Y2)))

#plt.legend(loc='best', fontsize=15)
plt.title("Relationship among Degree-day, Solar radiation and Precipitation", fontsize=18)
ax.set_xlabel('Degree-day during cultivation (celcius*day)', fontsize=16)
ax.set_ylabel("Amount of Solar radiation (MJ/m2*day)", fontsize=16)
ax.set_zlabel("Precipitation(50-60 days after seeding) (mm)", fontsize=16)
ax.view_init(0, 0)
plt.setp(ax.get_xticklabels(), fontsize=15, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=15, visible=True)
plt.setp(ax.get_zticklabels(), fontsize=15, visible=True)
#plt.savefig('181224_Dssat_png/181226_Precipitation_DAS60_70_VS_Yield_SSKT8605.png', bbox_inches='tight')
