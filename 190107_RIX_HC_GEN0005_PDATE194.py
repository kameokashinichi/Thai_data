...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  7 15:37:19 2019
...
"""
190107_Dssat spil-water relation file
http://ec2-52-196-202-21.ap-northeast-1.compute.amazonaws.com/cropsim/exec/2019-01-07T06-44-21-735Zde5283752855356b
soilID -> "HC_GEN0005"
...
soilW = []
with open(os.getcwd() + '/2019-01-07T06-44-21-735Zde5283752855356b/SoilWat.OUT', 'r') as f:
    for row in f:
...
sw_df3 = pd.DataFrame(s_stress[:, 1:], columns=columns[1:], index = newcol)
 
"""
190106 Summary.OUT
"""
record = []
with open(os.getcwd() + '/2019-01-07T06-44-21-735Zde5283752855356b/Summary.OUT', 'r') as f:
    for row in f:
...
sum_df3 = pd.DataFrame(summary[:, 1:], index=summary[:, 0], columns=col[1:])

"""
190106 SoilNi.OUT read Soil N Dssat file.
"""
soilN = []
with open(os.getcwd() + '/2019-01-07T06-44-21-735Zde5283752855356b/SoilNi.OUT', 'r') as f:
    for row in f:
...
sn_df3 = pd.DataFrame(sn_stress[:, 1:], columns=columns[1:], index = newcol)    
...
PN = []
with open(os.getcwd() + '/2019-01-07T06-44-21-735Zde5283752855356b/PlantN.OUT', 'r') as f:
    for row in f:
        PN.append(row.strip())
...
pn_df3 = pd.DataFrame(pn_stress[:, 1:], columns=columns[1:], index = newcol)

"""
190107 analyze the relation between yield(HWAH) and surface NO3(NI1D)
"""
info = sum_df3.loc[:, ["HDAT","HWAH", "PRCP"]].values.astype(np.float32)
sum_df3_1 = pd.DataFrame(info, index=sum_df3.index, columns=["HDAT","HWAH", "PRCP"])

order1 = sum_df3_1.sort_values("HWAH")["HWAH"].index

juv_no1 = sn_df3[sn_df3["DOY"]=="255"]
uni = []
...
hwah4 = sum_df3.loc[:, 'HWAH'].values.astype(np.int32)   #sum_df1.sort_values("HWAH")
...
dfs = sn_df3[sn_df3.index == wnum]
dfp = pn_df3[pn_df3.index == wnum]
...
plt.title("N stream between soil and plant in No" + repr(wnum)+"(yield="+sum_df3.loc[repr(wnum),"HWAH"]+"kg/ha)", fontsize=18)
...
"""
190108 check the relation between the unber of day which has raining and yield
with annotation -> http://nekoyukimmm.hatenablog.com/entry/2015/10/08/224607
"""
rain_day = []
for i in range(100):        
...
plt.legend(loc='best', fontsize=15)
plt.title("amount of rainy days(more than 20mm/day) VS Yield　-SSKT8601-", fontsize=18)
plt.xlabel('Number of rainy days during DOY200-250', fontsize=16)
plt.ylabel("Final Yield (kg/ha)", fontsize=16)
plt.setp(ax.get_xticklabels(), fontsize=15, visible=True)
plt.setp(ax.get_yticklabels(), fontsize=15, visible=True)
plt.savefig('190108_Reiton_RIX_png/190108_rainy_days_200_250doy_VS_yield_HC_GEN0005.png')
...
plt.setp(ax2.get_yticklabels(), fontsize=13, visible=True)
#ax.set_ylim(0, 30)
plt.savefig('190108_Reiton_RIX_png/190108_N_and_water_flow_HC_GEN0005.png', bbox_inches='tight')
plt.show()
...
"""
190108 PCA analysis with precipitation
"""
