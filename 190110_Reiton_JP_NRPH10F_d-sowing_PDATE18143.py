...
soilW = []
with open(os.getcwd() + '/2019-01-10T05-38-16-196Z0dd6e68de7570f4c/SoilWat.OUT', 'r') as f:
    for row in f:
...
sw_df6 = pd.DataFrame(s_stress[:, 1:], columns=columns[1:], index = newcol)
 
"""
190108 Summary.OUT
"""
record = []
with open(os.getcwd() + '/2019-01-10T05-38-16-196Z0dd6e68de7570f4c/Summary.OUT', 'r') as f:
    for row in f:
...
sum_df6 = pd.DataFrame(summary[:, 1:], index=summary[:, 0], columns=col[1:])

"""
190108 SoilNi.OUT read Soil N Dssat file.
"""
soilN = []
with open(os.getcwd() + '/2019-01-10T05-38-16-196Z0dd6e68de7570f4c/SoilNi.OUT', 'r') as f:
    for row in f:
...
sn_df6 = pd.DataFrame(sn_stress[:, 1:], columns=columns[1:], index = newcol) 
...
PN = []
with open(os.getcwd() + '/2019-01-10T05-38-16-196Z0dd6e68de7570f4c/PlantN.OUT', 'r') as f:
    for row in f:
        PN.append(row.strip())
...
pn_df6 = pd.DataFrame(pn_stress[:, 1:], columns=columns[1:], index = newcol)
...
PG = []
with open(os.getcwd() + '/2019-01-10T05-38-16-196Z0dd6e68de7570f4c/PlantGro.OUT', 'r') as f:
    for row in f:
        PG.append(row.strip())
...
pg_df6 = pd.DataFrame(pg_stress[:, 1:].astype(np.float32), columns=columns[1:], index = newcol)
...
#cumulative graphs for direct-sowing(18143)
order=sum_df6_1.sort_values("HWAH").index
ord_df = pd.DataFrame(np.arange(1,101), index=order, columns=['ORDER'])
sum_ds = pd.concat([sum_df6_1, ord_df], axis=1, sort=True)

val = sum_df6_1['HWAH'].values

fig = plt.figure(figsize=(10,5))
ax = fig.add_subplot(1,1,1)

yy1 = sum_df6_1.loc[order[50],'HWAH']*0.7
yy2 = sum_df6_1.loc[order[50],'HWAH']*0.2
Ind = np.abs(val - yy1).argmin()+1 

ax.hist(val, bins=50, cumulative=True, color='red', histtype="barstacked")
ax.plot([1000, 4300],[20, 20], color="black", label="20 percentile Yield="+repr(sum_df6_1.loc[order[20],'HWAH'])+"kg/ha")
ax.plot([1000, 4300],[50, 50], color="blue", label="50 percentile Yield="+repr(sum_df6_1.loc[order[50],'HWAH'])+"kg/ha")
ax.plot([1000, 4300],[80, 80], color="orange", label="80 percentile Yield="+repr(sum_df6_1.loc[order[80],'HWAH'])+"kg/ha")
