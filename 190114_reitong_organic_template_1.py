...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 13 06:57:02 2019
...
http://ec2-52-196-202-21.ap-northeast-1.compute.amazonaws.com/cropsim/exec/2019-01-12T04-17-38-650Z4da6662809bcab8b
soilid -> "JP_NRPH10F"
...
soilW = []
with open(os.getcwd() + '/2019-01-12T04-17-38-650Z4da6662809bcab8b/SoilWat.OUT', 'r') as f:
    for row in f:
...
sw_df8T = pd.DataFrame(s_stress[:, 1:], columns=columns[1:], index = newcol)
 
"""
190108 Summary.OUT
"""
record = []
with open(os.getcwd() + '/2019-01-12T04-17-38-650Z4da6662809bcab8b/Summary.OUT', 'r') as f:
    for row in f:
...
sum_df8T = pd.DataFrame(summary[:, 1:], index=summary[:, 0], columns=col[1:])

"""
190108 SoilNi.OUT read Soil N Dssat file.
"""
soilN = []
with open(os.getcwd() + '/2019-01-12T04-17-38-650Z4da6662809bcab8b/SoilNi.OUT', 'r') as f:
    for row in f:
...
sn_df8T = pd.DataFrame(sn_stress[:, 1:], columns=columns[1:], index = newcol) 
...
PN = []
with open(os.getcwd() + '/2019-01-12T04-17-38-650Z4da6662809bcab8b/PlantN.OUT', 'r') as f:
    for row in f:
        PN.append(row.strip())
...
pn_df8T = pd.DataFrame(pn_stress[:, 1:], columns=columns[1:], index = newcol)
...
PG = []
with open(os.getcwd() + '/2019-01-12T04-17-38-650Z4da6662809bcab8b/PlantGro.OUT', 'r') as f:
    for row in f:
        PG.append(row.strip())
...
pg_df8T = pd.DataFrame(pg_stress[:, 1:].astype(np.float32), columns=columns[1:], index = newcol)
